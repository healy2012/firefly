...
class NetLocalService(CommandService):
    def callTargetSingle(self,targetKey,*args,**kw):
        ...
        self._lock.acquire()
        try:
            target = self.getTarget(0) # call target 0
            if not target:
...
netlocalservice = NetLocalService("netlocalservice")
GlobalObject().netfactory.addServiceChannel(netlocalservice)

def net_localservice_handle(target):
    netlocalservice.mapTarget(target)
#end def
